  ...
  return (
    <section className="container mx-auto px-4 py-12 min-h-screen">
      ...
      {cartItems.length === 0 ? (
        ...
        <div className="space-y-4 md:space-x-8 flex flex-col lg:flex-row">
          {/* Cart Items */}
          <div className="w-full lg:w-2/3 bg-lightBackground rounded shadow p-4 gap-7">
            {cartItems.map((item) => (
              <div
                key={item.id}
                className="flex flex-col sm:flex-row items-center justify-between border rounded-sm shadow-md border-grayLight p-4 mb-4 relative"
              >
                {/* Product Details */}
                <div className="flex flex-row max-md:w-full gap-8">
                  <div className="w-40 sm:h-32">
                    <img
                      src={item.thumbnail}
                      alt={item.name}
          ...
          {/* Summary Section */}
          <div className="w-full lg:w-1/3 bg-lightBackground p-4 rounded shadow flex justify-between items-center">
